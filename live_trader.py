...
from risk_management import PositionSizer, StopLossManager, PortfolioRiskManager, RiskParams
...
class LiveTrader:
    ...
    def buy_with_atr(self, symbol: str, price: float, atr: float,
        ...
        quantity = self.position_sizer.atr_based_sizing(
            self.get_portfolio_value(), price, atr, risk_per_trade_pct=0.01
        )
        ...
        # Add to stop manager
        if trade:
            self.stop_manager.add_position(symbol, price, quantity)
